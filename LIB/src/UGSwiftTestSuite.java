...
public class UGSwiftTestSuite {
    ...
    // ── BruteForceBatching (Section 7: exact baseline for DP knapsack) ──────
    private static void testBruteForceBatching() {
        section("OptimisationEngine.bruteForceBatching (exact baseline, guards, edge cases)");

        // [NORMAL] Small instance where brute force can enumerate all subsets.
        DynamicArray<ServiceRequest> small = new DynamicArray<>();
        small.add(new ServiceRequest(1, 1, 2, "Documents", 5, 100, 110, "PENDING", -1)); // w=0.3, v=10
        small.add(new ServiceRequest(2, 1, 3, "Pizza", 2, 100, 200, "PENDING", -1));     // w=0.8, v=8
        small.add(new ServiceRequest(3, 1, 4, "Waakye", 1, 100, 300, "PENDING", -1));    // w=1.2, v=6
        // Capacity 1.0 kg. Feasible subsets: {1}(v=10), {2}(v=8), {1,2}(v=18, w=1.1>1.0 INFEASIBLE), {3}(v=6, w=1.2>1.0 INFEASIBLE)
        // Optimal = {1,2} is infeasible, so best feasible = {1} with value 10.
        DynamicArray<ServiceRequest> brute1 = OptimisationEngine.bruteForceBatching(small, 1.0);
        ok("bruteForceBatching selects the optimal feasible subset for a small instance",
                brute1.size() == 1 && brute1.get(0).getRequestId() == 1);

        // [NORMAL] DP and brute force must agree on the same small instance.
        DynamicArray<ServiceRequest> dpResult = OptimisationEngine.dpKnapsackBatching(small, 1.0);
        ok("dpKnapsackBatching and bruteForceBatching agree on the optimal subset (both pick request 1)",
                dpResult.size() == brute1.size()
                        && dpResult.get(0).getRequestId() == brute1.get(0).getRequestId());

        // [BOUNDARY] Input-size guard: more than 20 items returns empty.
        DynamicArray<ServiceRequest> big = new DynamicArray<>();
        for (int i = 0; i < 21; i++) {
            big.add(new ServiceRequest(i, 1, 2, "Food", 1, 100, 200, "PENDING", -1));
        }
        boundary("bruteForceBatching returns empty when input exceeds MAX_BRUTE_FORCE_ITEMS (21 > 20)",
                OptimisationEngine.bruteForceBatching(big, 10.0).isEmpty());

        // [INVALID] Zero capacity returns empty.
        invalid("bruteForceBatching with capacity 0 returns empty",
                OptimisationEngine.bruteForceBatching(small, 0.0).isEmpty());

        // [INVALID] Empty request list returns empty.
        invalid("bruteForceBatching with empty request list returns empty",
                OptimisationEngine.bruteForceBatching(new DynamicArray<>(), 1.0).isEmpty());

        // [INVALID] All requests overweight for the capacity.
        DynamicArray<ServiceRequest> heavy = new DynamicArray<>();
        heavy.add(new ServiceRequest(1, 1, 2, "Groceries", 1, 100, 500, "PENDING", -1)); // w=5.0
        invalid("bruteForceBatching excludes every request whose weight exceeds capacity",
                OptimisationEngine.bruteForceBatching(heavy, 1.0).isEmpty());
    }
